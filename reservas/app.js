...
const SETTINGS={testMode:true,version:'0.2.0'};
...
const DAYS=[
 ...
 {date:'2026-10-04',label:'Domingo 4',kind:'weekend',theme:'Río futuro · Gestión, conservación y cierre',highlights:'Vía navegable · Reservas urbanas · Áreas protegidas · Conversatorio · Peña de cierre'}
];

// Sólo aparecen aquí las actividades que el usuario debe o puede seleccionar al inscribirse.
// Los restantes contenidos del programa están incluidos en la entrada y no requieren reserva.
const ACTIVITIES=[
 {id:'foto-28',date:'2026-09-28',time:'18:30',type:'Taller para infancias',title:'Taller de Fotografía de Naturaleza · Mundos Escondidos',capacity:25,remaining:25,limited:true},
...
function bundlePrice(q,price){
 ...
 const options=[{size:1,cost:price.single},{size:3,cost:price.group},{size:4,cost:price.group}];
 const dp=Array(q+1).fill(null);dp[0]={cost:0,parts:[]};
 for(let i=1;i<=q;i++){
  for(const o of options){
   if(i<o.size||!dp[i-o.size])continue;
   const candidate={cost:dp[i-o.size].cost+o.cost,parts:[...dp[i-o.size].parts,o]};
   if(!dp[i]||candidate.cost<dp[i].cost||(candidate.cost===dp[i].cost&&candidate.parts.length<dp[i].parts.length))dp[i]=candidate;
  }
 }
 return dp[q];
}
...
function calculatePrice(){
 ...
 dates.forEach(date=>{
  const d=dayByDate(date),p=d.kind==='weekend'?PRICES.weekend:PRICES.weekday;
  total+=bundlePrice(state.paid,p).cost;base+=state.paid*p.single;
 });
 if(dates.length===DAYS.length){
  const full=bundlePrice(state.paid,PRICES.full);
  if(full.cost<total)return{total:full.cost,base,saving:Math.max(0,base-full.cost),description:'Pase completo aplicado automáticamente'};
 }
...
function pruneActivities(){
 ...
 Object.keys(state.activityQty).forEach(id=>{
  const a=ACTIVITIES.find(x=>x.id===id);
  if(!a||!dates.has(a.date))delete state.activityQty[id];
  else state.activityQty[id]=Math.min(state.activityQty[id],maxPlacesForActivity(a));
 });
}
function quotaText(a){
 if(a.unlimited)return'Sin cupo limitado';
 if(Number.isFinite(a.capacity)&&Number.isFinite(a.remaining))return `${a.remaining} de ${a.capacity} lugares disponibles`;
 if(Number.isFinite(a.capacity))return `${a.capacity} lugares`;
...
function renderDays(){
 ...
 host.querySelectorAll('.day-card').forEach(btn=>btn.addEventListener('click',()=>{
...
function renderActivities(){
 ...
 $('activitiesList').innerHTML=list.length?list.map(a=>{
  ...
  return `<article class="activity-row"><div><h4>${a.title}</h4><div class="activity-meta"><span>${d.label}</span><span>· ${a.time}</span><span>· ${a.type}</span><span class="quota ${quotaClass}">${quota}</span>${reservationTag}</div></div><div class="activity-counter"><button type="button" data-activity="${a.id}" data-delta="-1" aria-label="Quitar lugar">−</button><output>${q}</output><button type="button" data-activity="${a.id}" data-delta="1" aria-label="Agregar lugar">+</button></div></article>`;
 }).join(''):'<p>Para las fechas elegidas no hay talleres o capacitaciones que requieran selección previa.</p>';
...
function renderSummary(){
 ...
 $('summaryActivities').innerHTML=chosen.map(a=>`<span>${a.title} · ${state.activityQty[a.id]} lugar${state.activityQty[a.id]===1?'':'es'}</span>`).join('');
 const note=$('discountNote');
 if(price.saving>0){note.classList.remove('hidden');note.textContent=`Ahorro aplicado automáticamente: ${money(price.saving)}.`;}else note.classList.add('hidden');
}
function renderMode(){
...
function selectedActivitiesPayload(){return ACTIVITIES.filter(a=>Number(state.activityQty[a.id]||0)>0).map(a=>({id:a.id,fecha:a.date,horario:a.time,titulo:a.title,participantes:Number(state.activityQty[a.id]),cupo:a.capacity}));}
...
function validateForm(){
 if(!selectedDates().length)return'Elegí al menos una jornada.';
 if(!$('responsible').value.trim())return'Ingresá el nombre del responsable.';
 if(!/^\d{6,10}$/.test($('dni').value.replace(/\D/g,'')))return'Revisá el DNI del responsable.';
 if(!/^\S+@\S+\.\S+$/.test($('email').value.trim()))return'Revisá el correo electrónico.';
 if(!$('phone').value.trim())return'Ingresá un número de WhatsApp.';
 if(!$('city').value.trim())return'Ingresá la ciudad.';
 if(!$('terms').checked)return'Confirmá que revisaste la selección.';
 return'';
}
function escapeHtml(text){return String(text||'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));}
function renderTicket(data){
 const dates=data.fechas.map(date=>dayByDate(date).label).join(' · '),acts=data.actividades.map(a=>`${a.titulo} (${a.participantes})`).join('<br>')||'Sin talleres o capacitaciones seleccionados';
...
document.querySelectorAll('.mode-card').forEach(btn=>btn.addEventListener('click',()=>{
 ...
 if(state.mode==='full')DAYS.forEach(d=>state.selected.add(d.date));else state.selected.clear();
 state.activityQty={};renderAll();
}));
document.querySelectorAll('[data-counter]').forEach(btn=>btn.addEventListener('click',()=>{
 const delta=Number(btn.dataset.delta);
 if(btn.dataset.counter==='paid')state.paid=Math.max(1,Math.min(20,state.paid+delta));
 else state.free=Math.max(0,Math.min(20,state.free+delta));
 renderAll();
}));
$('reservationForm').addEventListener('submit',ev=>{
 ev.preventDefault();const error=validateForm(),message=$('formMessage');
 if(error){message.className='form-message error';message.textContent=error;return;}
 const data=collectData();message.className='form-message success';message.textContent='Prueba correcta: la selección puede convertirse en una pre-reserva. No se escribió nada en la planilla.';renderTicket(data);
});
